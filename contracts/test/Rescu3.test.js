...
describe("Rescu3", function () {
  ...
  it("should commit, reveal, and claim successfully", async function () {
    ...
    const hash = ethers.utils.keccak256(
      ethers.utils.defaultAbiCoder.encode(["address", "address", "uint256"], [addr1.address, addr2.address, nonce])
    );

    await contract.connect(addr1).commit(hash, { value: ethers.utils.parseEther("0.01") });
    await ethers.provider.send("evm_increaseTime", [60]);
    await contract.reveal(addr1.address, addr2.address, nonce);
